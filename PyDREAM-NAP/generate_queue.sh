...
for i in $(ls logs | grep "train" | grep -v "val"); do
  	...
  	TS_SOCKET=/tmp/theis $TS_EXECUTABLE python run_splitminer.py --log ./logs/$train_val_log --output_folder output_models --best_model best_models --n_threads $N_THREADS
	  if [ $current_host == "ctgpgpu8" ] || [ $current_host == "ctgpgpu7" ]
	  then
