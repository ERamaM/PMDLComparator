...
```{r}
ranking <- bPlackettLuceModel(data_acc, min=FALSE, nsim=2000, nchains=10, parallel=TRUE)
```
...
```{r}
mauro <- data_acc[, "Mauro"]
pasqua <- data_acc[, "Pasquadibisceglie"]
evermann <- data_acc[, "Evermann"]
```

## Mauro vs Pasquadibisceglie
```{r}
signed_test <- bSignedRankTest(mauro, pasqua, rope=c(-0.01, 0.01))
signed_test$posterior.probabilities
```

```{r}
plotSimplex(signed_test, A="Mauro", B="Pasquadibisceglie", plot.density=FALSE, alpha=0.5)
```

## Pasquadibisceglie vs Evermann
```{r}
signed_test <- bSignedRankTest(pasqua, evermann, rope=c(-0.01, 0.01))
signed_test$posterior.probabilities
```

```{r}
plotSimplex(signed_test, A="Pasquadibisceglie", B="Evermann", plot.density=FALSE, alpha=0.5)
```
...
```{r}
raw_data_acc <- read.csv("../processed_results/csv/raw_acc.csv", row.names=1)
```
...
```{r}
# Number 3 is the best performing one
...
```{r}
plotSimplex(results, A=names(index[3]), B=names(index[2]), posterior.label=TRUE, alpha=0.5)
```
...
## Segunda mejor aproximación vs tercera mejor aproximación
...
```{r}
# For some reason we need to switch the parameters to get the plot right ??
results <- bHierarchicalTest(matrix_2, matrix_1, rho=0.1, rope=c(-0.01, 0.01), nsim=20000, nchains=10, parallel=TRUE)
```

```{r}
plotSimplex(results, A=names(index[2]), B=names(index[1]), posterior.label=TRUE, alpha=0.5)
```
