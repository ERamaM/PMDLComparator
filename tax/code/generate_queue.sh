...
for i in $(ls ../data | grep  "train"); do
	  ...
	  if [ $current_host == "ctgpgpu8" ] || [ $current_host == "ctgpgpu7" ]
	  then
	    CUDA_VISIBLE_DEVICES=1 TMPDIR=$DUMP_DIR TS_SOCKET=/tmp/tax $TS_EXECUTABLE python train.py --fold_dataset ../data/$log --full_dataset ../data/$full_log --train --test --test_suffix --test_suffix_calculus
	  else
